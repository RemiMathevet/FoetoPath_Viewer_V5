...
let LDA_CLASSES = {};
let QUICK_PICKS = {}; // {organe: [{id, label, color}]}
...
fetch(_url('/api/config/labels')).then(r => r.json()).then(cfg => {
    ...
    if (cfg.quick_picks) QUICK_PICKS = cfg.quick_picks;
...
function renderDiagTags() {
    ...
    if (!container) return;
    const picks = QUICK_PICKS[state.tissueType] || [];
    if (picks.length === 0) { container.innerHTML = '<span style="font-size:10px;color:var(--text-muted);">Sélectionnez un tissu</span>'; return; }
    container.innerHTML = picks.map(p => {
        const sel = state.slideDiagnosis.includes(p.id) ? 'selected' : '';
        const short = p.label.length > 35 ? p.label.slice(0, 33) + '…' : p.label;
        return `<span class="ann-diag-tag ${sel}" title="${p.label}" onclick="toggleDiagTag('${p.id}')">${short}</span>`;
    }).join('');
}
...
async function annSave() {
    if (state.viewMode !== 'slide' || state.currentSlideIndex < 0) return;
    const foetDiag = [...state.organDiagnosis, ...state.signPicks, ...state.retentionPicks];
    ...
    try {
        const diagnosis = [...state.slideDiagnosis, ...state.organDiagnosis, ...state.signPicks, ...state.retentionPicks];
...
function labelRenderQuickTags() {
    ...
    for (const org of organs) {
        const quick = FOETO_QUICK_CACHE[org] || [];
        if (quick.length === 0) continue;
        const label = _ORGAN_LABELS[org] || org.charAt(0).toUpperCase() + org.slice(1);
        html += `<span style="font-size:9px;color:var(--text-muted);width:100%;margin-top:2px;">${label}</span>`;
        const grouped = {};
...
function _ctxSubmenu(organ, tree) {
    return tree.items.map(it => {
        if (it.action === 'normal') {
            return `<div class="ctx-menu-item accent" onclick="ctxLabelQuick('${organ}','normal')"><span class="ctx-check">&#10003;</span> ${it.label}</div>`;
        }
        const sel = state.labelDiagnoses.includes(it.id);
        return `<div class="ctx-menu-item ${sel ? 'selected' : ''}" onclick="ctxLabelQuick('${organ}','patho','${it.id}')"><span class="ctx-check">${sel ? '&#10003;' : ''}</span>${it.label}</div>`;
    }).join('');
}
...
function showContextMenu(x, y) {
    ...
    // Placenta labelling tree
    if (state.domain === 'placenta') {
        html += '<div class="ctx-menu-sep"></div>';
        for (const [organ, tree] of Object.entries(CTX_PLACENTA_TREE)) {
            html += `<div class="ctx-menu-sub"><div class="ctx-menu-item">${tree.label} &#9656;</div><div class="ctx-submenu">${_ctxSubmenu(organ, tree)}</div></div>`;
        }
    }
...
document.getElementById('viewer').addEventListener('contextmenu', e => {
    if (state.measureMode || state.annMode) return;
    e.preventDefault();
    e.stopPropagation();
